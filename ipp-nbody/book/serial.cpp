...
int main(int argc, char** argv){
    ...
    **/
	...
	for (int q = 0; q < n; q++){
		...
		if (DIM == 3){
		    long double z_diff = position[q][Z] - position[k][Z];
		    forces[q][Z] -= ((G * mass[q] * mass[k]) / dist3) * z_diff;
		}
	     }
	}
	for (int q = 0; q < n; q++){
	    ...
	    if (DIM == 3){
		position[q][Z] += timestep * velocity[q][Z];
		velocity[q][Z] += timestep / mass[q] * forces[q][Z];
	    }
	}
    }
    ...
    printOutput();
    //writeOutput();
    int result = ((finish.tv_sec - start.tv_sec) * 1000000L + finish.tv_usec) - start.tv_usec;
    cerr << result << " usec\n";
    //cout << "done.\n";
    return 0;
}
